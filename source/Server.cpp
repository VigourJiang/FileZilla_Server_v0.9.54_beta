...
/**
 * 处理来自Server Interface进程的命令。该命令通过CAdminSocket发送过来。
 */
...
bool CServer::CreateListenSocket()
{
	...
	bool success = CreateListenSocket(ports);
...
bool CServer::CreateListenSocket(CStdString ports)
{
	bool success = true;
	...
	while (pos > 0) {
		...
		while (ipBindings != _T("")) {
			...
			CStdString ip = ipBindings.Left(pos);
			ipBindings = ipBindings.Mid(pos + 1);
			CListenSocket *pListenSocket = new CListenSocket(*this, m_ThreadArray);
